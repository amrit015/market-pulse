...
| **Summary** (Daily Pulse) | The flagship AI-generated market briefing — a different narrative for market-hours, after-hours, and weekend. A pinned 7-day calendar strip (added 2026-09-07) lets the reader swipe/tap back through the last week's reports, each day cached independently once opened (a confirmed-empty past day, e.g. a weekend, is remembered too, so it's never re-fetched) | Markdown-rendered AI copy (Compose-Markdown), one of the few screens with sealed `Loading/Success/Error` UI state because it genuinely renders different layouts per state, `SummaryCalendarStrip` + `HorizontalPager` (see `@docs/architecture/collapsing-header-tabs.md`) |
