...
Every domain except the ones intentionally left one-shot-only caches its data in a single shared Room database, `AppDatabase` (currently schema **version 12**, `di/DatabaseModule.kt` builds it with `DatabaseMigrations.ALL_MIGRATIONS` applied — no destructive fallback).

- **Entities** (`storage/database/entity/`) mostly mirror the domain model 1:1. Two row-shapes exist:
  - *Singleton-per-day/id* (`IndicatorsEntity` keyed by `dateId`, `MarketPostureEntity`/`SummaryEntity` keyed by a fixed `id`) — one row holds the whole domain's current snapshot.
...
`ui/theme/Color.kt`'s `PulseStatusColors` object is the single source of bullish/bearish/neutral/warning colors (light/dark aware via `isSystemInDarkTheme()`), consumed either directly or through `utils/extensions/ColorExtension.kt`'s `SignalColor.toColor()/toBgColor()` when a domain model carries a `SignalColor` enum.
