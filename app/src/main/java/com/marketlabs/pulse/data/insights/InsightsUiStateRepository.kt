 ...
 * same shape `ThemeRepository` uses for exactly this reason (a domain that doesn't need the full
 * split). Backs the Positioning/Posture first-time explainer's persisted dismissal state --
 * there's no "seen it once" precedent anywhere else in the app to extend, so this introduces one
 * new DataStore instance using the same technology/idiom `ThemeRepository` already established,
