...
object DatabaseMigrations {
    ...
    // Migration from Version 29 to 30: `market_state.synthesisGeneratedAt` added -- backs the
    // Dashboard hero card's "Analyzed as of" line with `synthesis.generated_at` (when the AI pass
    // actually ran) instead of `lastUpdated` (the client's own local-sync timestamp). Purely
    // additive, no existing column touched.
    val MIGRATION_29_30 = object : Migration(29, 30) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `market_state` ADD COLUMN `synthesisGeneratedAt` INTEGER")
        }
    }
