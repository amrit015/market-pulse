 ...
 * per entry in `sections`, NOT one big card holding all of them ("Deep Dive" itself is a
 * page-level title outside any card, and each of the 9 sections is its own card). `kicker` is the
 ...
 *
 * `headlineStat`/`highlights`: structured quick-view data on every section, alongside the prose
 * `heading`/`body`, not a replacement for them. Stacking order is kicker -> heading -> body ->
 * `headlineStat` -> `highlights` -> `deltaChips` -- the AI-written narrative reads first, the
 * supporting figures after. An inset `HorizontalDivider` (inside this Column's own padding, not
 * full-bleed like a card's header divider) separates each populated block from the one before it
 * -- never between two blocks where one is absent, so a section with only `highlights` and no
 * `headlineStat` gets exactly one divider, not two. `headlineStat`'s value uses
 * `colorScheme.primary` so the one hero number reads as emphasized, not just big.
    ...
    PulseCard(style = PulseCardStyle.SYNTHESIS, modifier = modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(dimensionResource(id = R.dimen.padding_large))) {
            ...
            Text(
                ...
                // Mix/Watch/Risks all use plain `titleMedium`).
