 ...
 *
 * 2026-09: the Theme section no longer shows the full preset grid inline -- that's the dedicated
 * `ThemePickerScreen` now (2 sections, Light/Dark, reached by tapping the compact Theme row below,
 * which just previews the currently-selected preset). Every row -- Theme and each "More" item -- is
 * now its own `PulseCard` (was one shared card per section holding every row with dividers between
 * them).
 */
 ...
 * destinations — Data & Sync and About are static content screens; Notifications is still a
 * placeholder (content/design to follow separately). Share isn't a destination: it opens the
 * system share sheet. Terms & Conditions and Privacy Policy are their own top-level rows (no intermediate
