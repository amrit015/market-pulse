...
/**
 * The Detail screen's pinned header -- it stays fixed while everything below it
 ...
 *
 * `technicalSetup` and `regimeAtAnalysis` get deliberately different pill treatments:
 * `technicalSetup` (a classification, not a signal) is an outlined/unfilled pill in
 ...
 * (the backend's `previous_close`) and `intradayPoller` never touches it, so it
 ...
 *
 * Both badges are tap-to-explain, matching the same "tap a signal, see what it means"
 ...
 * share the word "setup") -- see `market_glossary.json`'s `stock_setups` section, backed by
