...
fun DirectNews(
    ...
    // 💡 No section header -- this renders on the News tab, whose title already says "News", so
    // a "DIRECT NEWS" header would be a redundant restatement.
...
private fun NewsCard(item: DomainStockNewsItem, isExpanded: Boolean, onToggle: () -> Unit, onArticleClick: (String) -> Unit) {
    ...
    PulseCard(
        ...
        // 💡 `animateContentSize()` sits on this Column, not on `PulseCard`'s own modifier -- it
        // clips whatever it wraps to its own animated rectangle each frame, and outside
        // `PulseCard`'s `Modifier.shadow` that clip cuts straight through the shadow's rounded
        // corners (a flat greyish sliver past the bottom edge instead of a clean rounded shadow).
        // Scoped to just this Column, only the card's own content is clipped as it grows/shrinks.
        //
        // 💡 Two padded blocks around the headline divider (Scenarios'/DeepStudy's merged-card
        // shape), so the headline/body divider stretches the card's full width rather than being
        // inset by `padding_large` on both sides.
            ...
            Column(modifier = Modifier.padding(dimensionResource(id = R.dimen.padding_large))) {
                ...
                if (isExpanded) {
                    item.forwardImplication?.let {
                        ...
                        // CardEyebrowLabel-style sub-label in the app uses (Digest, Scenarios,
                        // Deep Study).
                        Text(
