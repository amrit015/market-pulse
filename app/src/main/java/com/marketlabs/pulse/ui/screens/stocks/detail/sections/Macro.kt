...
/**
 * `PulseCard(DATA)`, like the other data cards (levels, fundamentals, macro, setup reasoning).
 * `rate_sensitivity` is a confirmed 4-value backend enum (`HIGH_NEGATIVE` /
 ...
 * is neutral. `excess_return_63d` exists on the domain model but isn't shown -- only
 * "excess return 21d" is displayed.
 */
@Composable
fun Macro(macro: DomainMacro?, modifier: Modifier = Modifier) {
    ...
    PulseCard(style = PulseCardStyle.DATA, modifier = modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(dimensionResource(id = R.dimen.padding_large))) {
            ...
            // a fixed 96dp cell per stat, so "Rate Sens" (a full-word value like "MODERATE
            // NEGATIVE", not a short number like its 3 siblings) expands to use the row it has to
            // itself as the lone leftover, instead of staying pinned to a narrow fixed column.
            val stats = listOfNotNull(
                macro.riskFree10yPct?.let {
                    StatItem(value = "${formatDecimal(it)}%", label = stringResource(id = R.string.stock_detail_metric_10y_yield))
                },
            ...
            StatGrid(stats = stats, modifier = Modifier.fillMaxWidth())
        }
    }
