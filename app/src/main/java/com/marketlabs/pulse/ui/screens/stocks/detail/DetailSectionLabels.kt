 ...
 *   bit of context on the same row ("tap a chip for why", "2 of 2 recent calls held"). Used
 *   instead of a page-level `SectionDividerLabel` (`titleLarge`/`accentPrimary`, sitting directly on
 *   the background between cards), since these sections sit on one `PulseCard` each
 ...
 *   Briefing card and `StockPreviewCard` already use elsewhere. Note: TechnicalRead uses
 *   [com.marketlabs.pulse.ui.components.widgets.CardEyebrowLabel] instead -- see that
 *   composable's own doc comment for why
...
fun DataCardSectionHeader(title: String, subtitle: String? = null, trailing: String? = null) {
    ...
    Column(modifier = Modifier.padding(dimensionResource(id = R.dimen.padding_large))) {
        ...
        // calls held" and Technical Timeline's "Last 30 days" read as a proper subtitle there
 ...
 * model). Takes its own [text] so the same structure covers every "View More"/"Read More"/"Open
 * full ..." CTA in the app (Resolved Calls, Technical Timeline, Deep Dive's "Open full Deep Dive")
 * instead of each one hand-rolling its own near-identical Row.
 */
@Composable
