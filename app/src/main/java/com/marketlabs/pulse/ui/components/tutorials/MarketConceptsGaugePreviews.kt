...
/**
 * spec-20260917-content-refinement.md Pass 4 follow-up (2026-09): the Sentiment & Positioning
 ...
 * Replaces [SentimentGaugesDiagram] (a hand-drawn 3-bar illustration covering Fear & Greed/
 * Put-Call/VIX) at this spot -- VIX isn't named anywhere in this section's own text, so it isn't
 * included here; that diagram is still defined and still valid, just no longer wired to this call
 * site.
 *
 * Each gauge carries its own short "how to read it" line directly underneath -- restores the
 * interpretive explanation [SentimentGaugesDiagram]'s own shared caption used to give (contrarian
 * framing, "extremes can persist," COT/AAII's own gotchas), broken out per-gauge instead of one
 * combined caption, since these are four real, separately-labeled widgets now rather than one
